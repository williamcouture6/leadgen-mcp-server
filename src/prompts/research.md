...
- **Facteur secondaire (amplifie, sans plafonner)**: 🔧 **le VOLUME de demandes entrantes**, pas la revente. Beaucoup d'avis, plusieurs métiers, plusieurs villes desservies, une équipe visible → plus d'appels qui rentrent, donc plus d'appels manqués, donc plus de valeur au palier **Essentiel** — celui qu'on vend.
  ⚠️ **Corrigé le 2026-08-30 (AC1b).** Ce facteur majorait auparavant sur les features du tier **Croissance** (« réactivation de la base dormante », « campagnes de renouvellement saisonnier »). C'était un barème qui priorisait des leads pour une offre qu'on ne leur présente pas : le courriel de tri vend l'entrée de gamme, et le palier Croissance est de l'upsell qui ne se discute qu'après la vente. Un lead noté haut pour sa base dormante mais qui reçoit peu d'appels était mal classé.
  Avis = proxy mou (1-10 % des clients en laissent) → un compte d'avis modéré ne disqualifie jamais à lui seul.
